...
const ProcessingListAll = () => {
    ...
    const handleDownloadCSV = () => {
        ...
        try {
            // Get data to download - either filtered or all data
            const dataToDownload = filteredBatches(processingBatches);
            
            // Create CSV headers
            const headers = ['Batch No', 'Total KGs', 'Grade', 'CWS', 'Processing Type', 'Status'];
            
            // Transform data to CSV format
            const csvData = dataToDownload.map(batch => [
                batch.batchNo || '',
                batch.totalKgs || '0',
                batch.grade || '',
                batch.cws?.name || '',
            ...
            link.setAttribute('download', `processing_batches_${new Date().toISOString().split('T')[0]}.csv`);
    ...
    const renderHeader = () => {
        return (
            <div className="mb-4 d-flex justify-content-between align-items-end" style={{ borderBottom: `2px solid ${processingTheme.primary}` }}>
                <div>
