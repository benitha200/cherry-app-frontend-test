...
const BaggingOffReport = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      ...
      const batchResponse = await axios.get(
        `${API_URL}/bagging-off/report/completed`,
        {
      ...
      if (batchResponse.data.reports) {
        ...
        setOverallMetrics((prev) => ({
          ...prev,
          totalNonNaturalInputKgs:
            batchResponse.data.overallMetrics.totalNonNaturalInputKgs,
          totalNonNaturalOutputKgs:
            batchResponse.data.overallMetrics.totalNonNaturalOutputKgs,
          overallNonNaturalOutturn:
            batchResponse.data.overallMetrics.overallNonNaturalOutturn,
        ...
        const stations = [
          ...new Set(
            reports.map((report) => (report.batchInfo || report).station)
          ),
        ];
        const processingTypes = [
          ...new Set(
            reports.map((report) => (report.batchInfo || report).processingType)
          ),
        ];
      ...
      const stationResponse = await axios.get(
        `${API_URL}/bagging-off/report/summary`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
      ...
      setError(
        err.response?.data?.error || "Error fetching bagging-off reports"
      );
  ...
  const applyFilters = (data) => {
    return data.filter((item) => {
      ...
      if (
        ...
        report.stationName !== filters.station
      ) {
      ...
      if (filters.processingType) {
        ...
        if (filters.processingType === "NATURAL") {
          ...
          if (
            !baseBatchHasNatural[baseBatchNo] &&
            report.processingType !== "NATURAL"
          ) {
            return false;
          }
        }
        ...
        // and matches the requested processing type
          ...
          if (
            baseBatchHasNatural[baseBatchNo] ||
            report.processingType !== filters.processingType
          ) {
            return false;
          }
        }
  ...
  const downloadTableAsExcel = () => {
    ...
    const filename = `bagging_off_${reportType}_report_${new Date()
      .toISOString()
      .slice(0, 10)}.xls`;
    let htmlContent = "";
      ...
      htmlContent = `
                <html>
                    ...
                    <body>
                        ...
                        <div>Overall Outturn (Without NATURAL): ${
                        ...
                        <table>
                            ...
                            <tbody>
                                ${filteredData
                                  .map((report) => {
                                    const batchInfo =
                                      report.batchInfo || report;
                                    const baseBatchNo = getBaseBatchNo(
                                      batchInfo.batchNo
                                    );
                                    const displayProcessingType =
                                      baseBatchHasNatural[baseBatchNo]
                                        ? "NATURAL"
                                    ...
                                    <tr>
                                        ...
                                        <td>${
                                          batchInfo.totalInputKgs ||
                                          batchInfo.totalKgs
                                        }</td>
                                        <td>${
                                          batchInfo.totalOutputKgs ||
                                          batchInfo.totalKgs
                                        }</td>
                                  ...
                                  .map(
                                    ...
                                    <tr>
                                        ...
                                        <td className="text-end">${(
                                          ...
                                          summary.nonNaturalInputKgs
                                        ).toLocaleString()}</td>
                                        <td className="text-end">${(
                                          summary?.totalOutputKgs -
                                          summary.nonNaturalOutputKgs
                                        ).toLocaleString()}</td>
                                        <td
                                            className="text-end"
  ...
  const downloadTableAsCSV = () => {
    ...
    if (reportType === "batch") {
      ...
      csvData = filteredData.map((summary, index) => [
        index + 1,
        summary.stationName,
        summary.nonNaturalInputKgs,
        summary.nonNaturalOutputKgs,
        summary.outturn + "%",
        summary?.totalInputKgs - summary.nonNaturalInputKgs,
      ...
      <table className="table table-hover">
        ...
        <tbody>
          {loading ? (
            Array(10)
              .fill(0)
              .map((_, index) => <SkeletonRow key={index} cols={10} />)
            ...
            currentItems.map((report, index) => {
              ...
              return (
                <tr
                  ...
                  style={{
                    backgroundColor:
                      index % 2 === 0 ? "white" : theme.tableHover,
                    cursor: "pointer",
                  }}
                ...
                >
                  ...
                  <td
                    style={{
                      textAlign: "right",
                      color:
                        displayProcessingType === "NATURAL"
                          ? "teal"
                          : batchInfo.outturn >= 20.5 && batchInfo.outturn <= 25
                          ? theme.primary
                          : "red",
        ...
        {loading ? (
          ...
          currentItems.map((summary, index) => (
            <tr
              key={index}
              style={{
                backgroundColor: index % 2 === 0 ? "white" : theme.tableHover,
                cursor: "pointer",
              }}
            ...
            >
              ...
              <td>{summary.stationName}</td>
              <td className="text-end">
                {summary.nonNaturalInputKgs.toLocaleString()}
              </td>
              <td className="text-end">
                {summary.nonNaturalOutputKgs.toLocaleString()}
              </td>
              <td
                className="text-end"
                style={{
                  textAlign: "right",
                  color:
                    summary.outturn >= 20.5 && summary.outturn <= 25
                      ? theme.primary
                      : "red",
                  backgroundColor: theme.neutral,
                }}
      ...
      <div className="card">
        <div className="card-body">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h4 className="mb-0">
              {reportType === "batch"
                ? "Batch Out Turns Report"
                : "Station Bagging Off Summary"}
            </h4>
